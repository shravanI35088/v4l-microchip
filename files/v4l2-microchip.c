...
struct skeleton {
	...
	struct v4l2_subdev_entity	*current_subdev;
...
static int v4l2_parse_dt(struct device *dev, struct skeleton *skel)
{
	...
	INIT_LIST_HEAD(&skel->subdev_entities);
...
static int v4l2_async_bound(struct v4l2_async_notifier *notifier,
			    ...
			    struct v4l2_async_subdev *asd)
{
	...
	if (video_is_registered(&skel->vdev)) {
		v4l2_err(&skel->vdev, "only supports one sub-device.\n");
		return -EBUSY;
	}
...
static void v4l2_async_unbind(struct v4l2_async_notifier *notifier,
			      struct v4l2_subdev *subdev,
			      struct v4l2_async_subdev *asd)
{
	struct skeleton *skel = container_of(notifier->v4l2_dev,
...
static int v4l2_async_complete(struct v4l2_async_notifier *notifier)
{
	...
	if (ret < 0) {
		v4l2_err(&skel->v4l2_dev, "Failed to register subdev nodes\n");
		return ret;
	}
...
const struct v4l2_async_notifier_operations v4l2_async_ops = {
	.bound = v4l2_async_bound,
	.unbind = v4l2_async_unbind,
	.complete = v4l2_async_complete,
};

void v4l2_subdev_cleanup(struct skeleton *skel)
{
	struct v4l2_subdev_entity *subdev_entity;

	list_for_each_entry(subdev_entity, &skel->subdev_entities, list) {
...
static int v4l2_microchip_probe(struct platform_device *pdev)
{
	...
	list_for_each_entry(subdev_entity, &skel->subdev_entities, list) {
		...
		subdev_entity->notifier.ops = &v4l2_async_ops;
